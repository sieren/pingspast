    ...
    namespace detail
    {
        ...
        class future_waiter
        {
            ...
            struct all_futures_lock
            {
                ...
                all_futures_lock(std::vector<registered_waiter>& ftrs):
                    count(ftrs.size()),locks(new boost::unique_lock<boost::mutex>[count])
                {
                    for(count_type_portable i=0;i<count;++i)
                    {
                        locks[i]=BOOST_THREAD_MAKE_RV_REF(boost::unique_lock<boost::mutex>(ftrs[i].future_->mutex));
                    }
                }
