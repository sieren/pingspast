...
#ifdef BOOST_CLANG
    //NI_PATCH workaround for LLVM bug 14074 
    // std::streambuf::sputn is broken in libc++, using sputc instead
    const Elem *s = static_cast<const Elem *>(address);
    std::size_t scount = 0;
    while(scount < count && m_sb.sputc(*s++) != Tr::eof())
        ++scount;
    if(count != scount)
#else // BOOST_CLANG
